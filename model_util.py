...
# model_util.py -> HierarchicalQuantizer        # ...
...
class SwinStage(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if downsample is not None: # PatchMerging layer
...
class SwinEncoder(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
        ...
        self.num_features = int(embed_dim * 2**(self.num_layers - 1)) # Num features at output of last stage
        self.mlp_ratio = mlp_ratio

        # Split image into non-overlapping patches
        self.patch_embed = TimmPatchEmbed( # Using timm's PatchEmbed
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        
        num_patches = self.patch_embed.num_patches
        self.patches_resolution = self.patch_embed.grid_size # Or patches_resolution
    ...
    def forward(self, x: torch.Tensor, encoder_boolean_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        # encoder_boolean_mask is not directly used by Swin like in MAE,
        # Swin processes the full image through its stages.
        # If masking is needed, it's usually done before patch_embed or by modifying token inputs.
        # For Approach 1 with mask_ratio=0, this is fine.
        if encoder_boolean_mask is not None and encoder_boolean_mask.any():
            print("Warning: SwinEncoder received an encoder_mask_boolean, but standard Swin processes the full image. Masking not applied in Swin stages.")

        x = self.patch_embed(x) # Output: B, L, C (L = num_patches)
        if self.ape:
        ...
        x = self.norm(x)  # B L C (L here is num_patches after last stage's downsampling)
        # Output shape: [B, (H/P/2^num_stages_with_downsample) * (W/P/2^num_stages_with_downsample), C_last_stage]
        return x